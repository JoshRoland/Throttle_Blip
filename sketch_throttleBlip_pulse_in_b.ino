...
int brake;                       // brake pedal depressed
int brakepin = 4;                // digital input pin for brake postion

// Clutch Indicator Switch
int clutch;                      // clutch pedal depressed
int clutchpin = 5;               // digital input pin for clutch position

// Vehicle Speed Settings
int speedPin = 3;                // digital input pin used to measure speed of vehicle
float speed_trigger = 4.725;     // 40cm/pulse based in OEM tire size
unsigned long high_time_speed;   // time duration when speed pulse is in high state
unsigned long low_time_speed;    // time duration when speed pulse is in low state
...
float period_time_speed;         // used to calc one full period time of speed pulse
float freq_speed;                // frequency (Hz) of speed pulse
...
unsigned long high_time_rpm;     // time duration when rpm pulse is in high state
unsigned long low_time_rpm;      // time duration when rpm pulse is in low state
float period_time_rpm;           // used to calc one full period time of rpm pulse
float freq_rpm;                  // frequency (Hz) of rpm pulse
...
// Linear regression of engine RPM (no load) as a result of VPA (throttle) voltage
...
float VPA2_offset = 0.87;         // Voltage offset between VPA and VPA2
...
void setup() {                        // put your setup code here, to run once:
  ...
  // Set Adruino DAC throttle voltage (VPA and VPA2) DAC output to 0
...
void loop() {                           // put your main code here, to run repeatedly:
  ...
  brake = digitalRead(brakepin);        // read output from brake pedal
  clutch = digitalRead(clutchpin);      // read output from clutch pedal

  // Measures vehicle speed
  high_time_speed = pulseIn(speedPin,HIGH);  // measure the pulse duration while in high (1) state
  low_time_speed = pulseIn(speedPin,LOW);    // measure the pulse duration while in the low (0) state
  ...
  if (brake == 1 && clutch == 1 && vSpeed >= 25 && tRPM >> RPM && (gear == 2 || gear == 3 || gear == 4))
  {
    ...
    VPA2 = VPA + VPA2_offset;                          // Sets target output voltage for VP2 (DAC channel B)
  ...
  else
  {
    ...
    VPA2 = VPA + VPA2_offset;                          // Sets target output voltage for VP2
    unsigned long VPA2dac = VPA2 * 65536 / Vcc;        // Converts voltage to 16 bit
    int dac_B_byte_1 = VPA2dac / 256;                  // Calculates value for 1st 8bit frame to send to DAC
    int dac_B_byte_2 = VPA2dac - (dac_B_byte_1 * 256); // Calculates value for 2nd 8bit frame to send to DAC
